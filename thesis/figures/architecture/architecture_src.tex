...
\node[box, text width=2.7cm]  (spat) at (3.4,0)    {\ttl{Spatial Tokens}\\[1pt]\sub{Fourier/RFF encoding,\\ residual MLP}};
\node[picon, text width=2.6cm](cfc)  at (6.8,0)    {\btl{CfC Memory}\\[1pt]\sub{token attention,\\ causal scan dt}};

% ---- trunk row ----
\node[box, text width=2.4cm]  (qry)  at (3.4,-3.2) {\ttl{Query}\\[1pt]\sub{$(x,y,t_q,c)$}};
\node[box, text width=2.7cm]  (tf)   at (6.8,-3.2) {\ttl{Trunk Feature}\\[1pt]\sub{Fourier + temporal anchor,\\ dt\_to\_query}};

% ---- interaction ----
\node[picon, text width=2.9cm](ca)   at (10.5,-1.6){\btl{Cross-Attention Readout}\\[1pt]\sub{isotropic distance bias}};

% ---- output ----
\node[box, text width=2.6cm]  (bb)   at (14.0,0.0) {\ttl{Branch Basis}\\[1pt]\sub{query-conditioned context}};
\node[box, text width=2.6cm]  (tb)   at (14.0,-3.2){\ttl{Trunk Basis}\\[1pt]\sub{component-wise basis}};
\node[box, text width=2.8cm]  (fus)  at (17.2,-1.6){\ttl{DeepONet Fusion}\\[1pt]\sub{branch $\times$ trunk basis,\\ component-wise output}};
\node[box, text width=1.7cm]  (fld)  at (20.0,-1.6){\ttl{Field}\\[1pt]\sub{$u,v,p$}};
\node[picon, text width=3.0cm](loss) at (19.6,-4.6){\btl{Loss}\\[1pt]\sub{AL continuity constraint,\\ sensor MSE + NS residual, GradNorm}};

% ---- optimizer feedback ----
\node[box, text width=3.2cm]  (mp)   at (8.2,-6.8){\ttl{Model Parameters}\\[1pt]\sub{encoder + CfC + decoder}};
\node[box, text width=3.2cm]  (opt)  at (14.5,-6.8){\ttl{SOAP + Schedule-Free}\\[1pt]\sub{preconditioned updates}};
...
\node[zone] at (9.2,1.5) {INTERACTION};
\node[zone] at (13.0,1.5) {OUTPUT AND LOSS};
...
\node[font=\sffamily\bfseries\small, text=inkline, anchor=west] at (14.6,-8.15) {Full-field DNS is not used as supervision.};
