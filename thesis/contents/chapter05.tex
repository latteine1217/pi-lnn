...
\begin{itemize}
    ...
    \item \textbf{(O3) Placement and noise set reliability.} Three placement strategies (DNS-oracle, DNS-free LES, random) all keep KE error within the engineering target, with placement contributing a variance source $6.4\times$ training stochasticity; additive sensor noise up to $10\%$ keeps the reconstruction within the target in the noise-stress test.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{CFD-rigour gaps.} The DNS passes Pope-2000 ($k_{\max}\eta=1.91$) but $T/t_{\rm eddy}=2.51$ gives a short statistical window. Internal $p$ is unsupervised (Appendix~\ref{app:pressure_limit}).
...
\begin{enumerate}
    ...
    \item \textbf{4D-Var assimilation and pressure-supported reconstruction.} Condition a forward solver on the continuous sensor stream via 4D-Var, and add pressure taps or a pressure-Poisson decoder for applications requiring pressure accuracy.
