...
Figure~\ref{fig:crossre_sensor_budget} reports the outcome. The budget needed to reach the $10\%$ engineering threshold rises with Reynolds number: $K \le 10$ at $Re=10^{2}$, $K \in (10, 50]$ at $Re=5\times10^{2}$ and $10^{3}$, $K \in (50, 100]$ at $Re=10^{4}$, and $K > 100$ at $Re=10^{6}$. The ordering follows the sampling-band argument of \S\ref{subsec:nyquist_scaling}: what a fixed budget must cover is the spectral support of the field, and the saturated bandwidth of these flows widens with $Re$. Reading the required budget against the band edge $K \gtrsim \pi n_{99}^{2}/2$, where $n_{99}$ is the integer wavenumber enclosing $99\%$ of the energy, places $Re=10^{4}$ ($n_{99}\approx6$) at $K\approx57$, consistent with the measured interval $(50, 100]$.

Two boundaries limit the reading. Each point is single-seed, so the curves carry no seed statistics and the threshold crossings are bracketed by the tested grid rather than resolved within it. The $Re=10^{2}$ case relaxes to the steady laminar Kolmogorov solution over the matched window, and its reconstruction is therefore not representative of the turbulent regime; it is retained as the laminar end member. At $K=10$ every turbulent case exceeds $40\%$ error, and differences within that range are not interpreted.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.72\linewidth]{figures/results/crossre_sensor_budget.pdf}
    \caption{Kinetic-energy relative error (dimensionless, reported in percent) against sensor count $K$ (dimensionless) at five Reynolds numbers. All runs use farthest-point placement, an observation window of five eddy turnover times, $d_\text{model}=256$, $20\,000$ iterations, $1\,024$ collocation points, and seed 42; reference fields are resolved DNS. The dotted line marks the $10\%$ threshold. Metrics are evaluated over the full observation window. Single seed per point.}
