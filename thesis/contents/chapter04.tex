...
\begin{table}[!htbp]
    ...
    LES-derived QR-pivot & No & $5.71 \pm 0.12$ & $\mathbf{13.65 \pm 0.06}$ & main deployable placement \\
...
\end{equation}
The ratio in \eqref{eq:placement_dominance} shows that LES-derived placement improves both the mean (KE $5.71\%$ vs.\ $7.95\%$, $\Delta=2.24$ percentage points, Welch $z \approx 3.3$, $p<0.01$) and the reproducibility ($6.4\times$ tighter), so engineering deployment should prioritise placement optimisation over training repetition. Divergence ratio stays at $0.35 \pm 0.01\%$ across all five placement seeds: the divergence diagnostic (\S\ref{sec:sub_dns_div}) is placement-invariant within the random-uniform family.
