...
This chapter reports the main reconstruction result, the studies that validate it, and a systematic account of how the sensing configuration governs reconstruction quality. The pipeline is: sensor placement $\to$ sensor measurements (here, DNS samples) $\to$ sensor-only physics training $\to$ continuous field reconstruction; placement uses a low-cost LES, one of three placement strategies studied. The chapter proceeds from the headline result, its field and spectral characterisation, and the architectural ablation that supports it; through a sensing-configuration study of sensor count, placement, and noise; to a higher-Reynolds feasibility case, diagnostic boundaries, and inference cost.

\section{Main Result on $Re=10\,000$ Kolmogorov Flow}
\label{sec:main_result}

The main result uses the DNS-free LES-derived placement (one of the three placement strategies studied; \S\ref{sec:placement_analysis}): a one-head PI-CON readout, $1024$ physics-collocation points, and $K=100$ sensor locations selected from a statistically-converged LES trajectory rather than DNS. Both the LES-pivot and DNS-pivot configurations are evaluated over the same $n=5$ random seeds at $20\,000$ training iterations, differing only in the sensor-placement source; Table~\ref{tab:main_metrics} therefore isolates placement quality.
...
The main row in Table~\ref{tab:main_metrics} uses no DNS full field for sensor placement or training supervision; the sensor values remain the DNS stand-in sampled at the $K$ positions. Against the DNS-oracle reference the two placements trade integral- against pointwise-fidelity: the DNS oracle attains lower KE error ($4.68\%$ vs $5.71\%$), while LES-derived placement attains lower pointwise velocity error ($u$ rel-$L_2$ $13.65\%$ vs $15.34\%$, $v$ rel-$L_2$ $17.52\%$ vs $18.10\%$). Neither placement dominates; LES-derived placement is thus competitive with the DNS oracle while requiring no DNS full field for placement. Across the full placement spectrum, reconstruction stays engineering-grade even without placement effort: random placement attains KE $7.95\%$ (five placement seeds, fixed training seed; \S\ref{sec:placement_analysis}), LES-derived placement improves this to $5.71\%$, and the non-deployable DNS oracle reaches $4.68\%$, all within the $10\%$ target, so reconstruction quality does not depend on a privileged sensor layout. Among DNS-free options, LES-derived placement is preferred: it lowers KE error by $2.24$ percentage points over random and reduces placement-induced variance roughly sixfold (\S\ref{sec:placement_analysis}); it is adopted as the reference configuration throughout, since placement governs reliability, not feasibility.

The divergence ratio of $0.39\%$ confirms the continuity constraint is active, matching the finite-difference floor of a DNS field band-limited to the reconstruction's resolved bandwidth (\S\ref{sec:sub_dns_div}; Fig.~\ref{fig:main_trajectories}, middle). The energy-dominant low-wavenumber band (sensor Nyquist cutoff $k_{\max}^{\rm sensor}\approx 5.64$, carrying $\approx 99\%$ of kinetic energy at $t=5$; \S\ref{subsec:nyquist_scaling}) recovers to single-digit relative error (Fig.~\ref{fig:band_err}), within the $10\%$ engineering target of O1.

Error concentrates at $t\in[0, 0.1]$, consistent with the difficulty of reconstructing the initial condition; for $t\ge 1$ the KE error stabilises at $\sim 5\%$ (Fig.~\ref{fig:main_trajectories}, top and bottom). The sensor-count sweep in \S\ref{subsec:nyquist_scaling} is single-seed, but uses the final PI-CON protocol and is interpreted only as a bandwidth trend.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/results/main_trajectories.pdf}
    \caption{LES-pivot main-pipeline trajectory diagnostics over $n=5$ seeds (42, 1, 2, 3, 4), versus time $t$ (s). Top: kinetic energy $\mathrm{KE}(t)$ (m$^2$/s$^2$). Middle: divergence ratio $\Vert\nabla\!\cdot\!\mathbf{u}_{\rm pred}\Vert_2 / \Vert\nabla\mathbf{u}\Vert_F^{\rm DNS}$ (\%), with the DNS finite-difference floor. Bottom: $u$ and $v$ relative $L_2$ errors (\%). In each panel the dashed line is the across-seed mean, dotted lines are individual seeds, and the shaded band is $\pm 1\sigma$; the DNS reference is solid.}
...
The main result is checked against seed variability with an $n=5$ ensemble, which also provides a training-variability estimate before the per-seed values are reported.
...
The architectural advantage holds at budget parity. With all four ablation cells trained at the matched $1\,024$-collocation $20\,000$-iteration $n=5$-seed regime, a Welch $t$-test between B0 (Vanilla DeepONet, $8.23 \pm 0.22\%$) and B3 (Full PI-CON, $5.71 \pm 0.11\%$) gives a KE gap of $-2.53$ percentage points ($t=22.9$, $p=3.0\times10^{-7}$, Cohen's $d=14.5$): the gain is significant and not an artefact of seed count or iteration budget. The full $2\times2$ component decomposition appears with the ablation matrix (\S\ref{sec:baseline_comparison}).
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/results/field_comparison_t5.png}
    \caption{Velocity-field comparison at $t=5$ (seed 42). Columns: DNS reference, PI-CON prediction, absolute error. Rows: $u$ (m/s), $v$ (m/s), vorticity $\omega$ (1/s).}
    \label{fig:field_t5}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.92\linewidth]{figures/results/vorticity_comparison_t5.png}
    \caption{Vorticity $\omega = \partial_x v - \partial_y u$ (1/s) at $t=5$ (seed 42). Left: DNS; middle: PI-CON prediction; right: error $\omega_{\rm pred} - \omega_{\rm DNS}$ (1/s). Axes: $x, y$ on $\Omega=[0,1]^2$ m$^2$. Left and middle share a colourbar; the error panel uses an independent symmetric scale.}
    \label{fig:vorticity_t5}
\end{figure}

Figure~\ref{fig:field_t5} compares $u$, $v$, and $\omega$ at $t=5$. PI-CON reproduces the dominant DNS structures (the large-scale shear bands in $u$ and energy-containing vortices in $v$) at the correct positions, signs, and amplitudes; the error panels (right column) show only fine-scale, structured residual rather than a bulk offset. The reconstruction acts as a low-pass copy: the energy-dominant large scales are recovered while the finest features are attenuated. This is clearest in vorticity (Figure~\ref{fig:vorticity_t5}, which amplifies high wavenumbers by $k^2$): PI-CON captures the principal vortices (their location, rotation sense, and gross shape) but blurs thin filaments and sharp cores, under-shooting peak vorticity magnitude. The error panel shows that the residual traces vortex boundaries and high-shear filaments rather than being spatially random, consistent with the spectral roll-off in \S\ref{sec:spectrum}.

\paragraph{Discussion.}
The error structure in Figures~\ref{fig:field_t5}--\ref{fig:vorticity_t5} is consistent with $k^2$ amplification of $\omega$ relative to $u$ and $v$: vorticity is the hardest quantity to reconstruct under the $K=100$ budget and is reported as a diagnostic rather than a primary observable. The spatially structured residual, rather than random noise, indicates a wavenumber-content limitation, in agreement with the per-band analysis in \S\ref{sec:spectrum}. For engineering tasks driven by mean flow, kinetic energy, and dominant-structure positions, the $u$ and $v$ reconstruction in Figure~\ref{fig:field_t5} is adequate; tasks requiring fine-scale turbulence statistics (such as higher-order Reynolds-stress moments) need either more sensors or a different prior.

\paragraph{Low-order statistics.}
Beyond the instantaneous fields, the time- and span-averaged statistics quantify how well the reconstruction serves mean-flow monitoring. Figure~\ref{fig:mean_profile} reports the mean streamwise profile $\langle u\rangle_{x,t}(y)$ and the Reynolds shear stress $\langle u'v'\rangle_{x,t}(y)$, averaged over $x$ and the post-spin-up window $t\in[1,5]$~s. The mean profile agrees with DNS to $1.55\%$ relative $L_2$ error and the Reynolds shear stress to $8.61\%$; the two-lobe shear structure set by the $k_f=2$ forcing is reproduced in both amplitude and position. Low-order statistics are thus recovered well within the engineering target, supporting mean-flow and momentum-flux monitoring at the $K=100$ budget.
...
Figure~\ref{fig:band_err} reports the per-band relative error against time. The low band ($k\le 5$) stays near $\sim 4\%$ throughout and reaches near-zero at several instants; the mid-$k$ and high-$k$ bands saturate near $100\%$ from $t\gtrsim 0.5$\,s, at the sensor-information ceiling. (The $t=0$ frame is excluded because high-$k$ energy there is near-zero, making relative error singular.)

\paragraph{Discussion.}
The high-$k$ deviation is not a model defect; it is the expected signature of a sensor-limited reconstruction. At $K=100$ the available measurements constrain the energy-dominant low-wavenumber band but do not independently determine the mid and high bands. The $K$-scaling experiment in \S\ref{subsec:nyquist_scaling} provides further evidence: the reconstruction bandwidth tracks the Nyquist-predicted wavenumber ceiling.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/results/band_energy_rel_error_vs_time.pdf}
    \caption{Band-energy relative error (dimensionless) vs.\ time $t$ (s), seed 42, LES-derived placement. Bands: low-$k$ ($k\le 5$, sensor Nyquist cutoff), mid-$k$ ($5<k\le 16$), high-$k$ ($k>16$). The $t=0$ frame is excluded.}
    \label{fig:band_err}
\end{figure}

\paragraph{Enstrophy and dissipation under the spectral cutoff.}
The band-limited reconstruction carries a direct consequence for vorticity-derived statistics. Figure~\ref{fig:dissipation_budget} compares the enstrophy $\mathcal{Z}(t)$ against DNS, with the dissipation rate $\varepsilon=2\nu\mathcal{Z}$ on the right axis as a constant rescaling. The reconstruction lies below the DNS reference by $26.4\%$ on the post-spin-up window $t\ge 1$~s, because the mid/high-wavenumber content removed by the $K=100$ Nyquist cutoff carries enstrophy weighted by $k^2$. The deficit is consistent with the spectral roll-off at $k\ge 16$ (Fig.~\ref{fig:spectrum}) and the $\omega$ relative error of Table~\ref{tab:main_metrics}: kinetic energy, concentrated at low wavenumbers, is recovered, while enstrophy-level statistics remain sensor-bounded.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/results/dissipation_budget.pdf}
    \caption{Enstrophy $\mathcal{Z}(t)$ (left axis, 1/s$^2$) versus time $t$ (s), seed 42, LES-derived placement; the right axis shows the dissipation rate $\varepsilon=2\nu\mathcal{Z}$ (m$^2$/s$^3$), a constant rescaling of the left axis. Solid: DNS; dashed: PI-CON.}
    \label{fig:dissipation_budget}
\end{figure}

\paragraph{Forcing-mode ($k = k_f = 2$) diagnostic.}
Because $k_f = 2$ lies inside the sensor-resolvable low band, the forcing-mode amplitude and phase serve as a focused sanity check on the energy-injection scale rather than as broadband evidence. Figure~\ref{fig:kf_mode} tracks both over $n=5$ seeds: after the initial-condition window ($t\ge 1$~s) the amplitude ratio holds near unity (across-seed mean $0.99$) and the phase error stays within $0.09$~rad ($\approx 5^\circ$), ending at $-0.013$~rad, the larger early-time excursions reflecting the initial-condition reconstruction difficulty. The forcing enters the Navier--Stokes residual as a fixed source, yet the velocity coefficient $\hat{u}_{k_f}(t)$ is a dynamical response whose decaying amplitude and phase the model recovers; broadband pointwise fidelity is quantified separately by the velocity relative-$L_2$ errors of Table~\ref{tab:main_metrics}.
...
Three sensing-configuration levers govern reconstruction quality: sensor count, placement, and measurement noise. This section quantifies the sensor count and its Nyquist information ceiling, then reports placement and noise robustness.
...
\begin{table}[!htbp]
    \centering
    \caption{Sensor-count scaling and Nyquist reconstruction ceiling. LES-derived QR-pivot sensors, seed 42, $20\,000$ iterations; metrics at $t=5$. The $K=400$ run uses $512$ collocation points (memory budget).}
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption{Radial energy spectrum $E(k)$ (m$^3$/s$^2$) versus wavenumber $k$ (1/m) at $t=5$ for the three sensor budgets of Table~\ref{tab:k_scaling_nyquist}. Each panel: DNS (solid black), PI-CON (dashed orange), $k^{-3}$ enstrophy-cascade reference (dotted grey; the forward-cascade slope for $k>k_f$), forcing $k_f = 2$ (dash-dotted black), sensor Nyquist $k_{\max}^{\rm sensor}=\sqrt{K/\pi}$ (dashed green). LES-derived QR-pivot, seed $42$, $20\,000$ iterations.}
...
The $K=100$ result should be read against the sensor information ceiling rather than as an unconstrained full-field regression problem. A wavelet sparsity diagnostic, a sensing-matrix rank calculation, and a spectral-truncation bound all give the same conclusion: the low-wavenumber band is observable enough to support engineering KE and mean-flow monitoring, while mid- and high-wavenumber content remains underdetermined at this sensor budget. The B3 multi-seed KE error ($5.71\%$) maps to an effective spectral cutoff between $k_{\rm cut}=4$ and $5$, and the vorticity error maps to a cutoff near the sensor Nyquist estimate $k_{\max}^{\rm sensor}\approx 5.64$. Thus, closing the remaining mid/high-band error requires more sensors or extra modalities, not another architecture-side loss term.
...
All three strategies stay below the $10\%$ KE engineering target. The DNS oracle gives the lowest integral KE error ($4.68\%$), but the LES-derived placement gives lower pointwise velocity error ($u$ rel-$L_2$ $13.65\%$ vs.\ $15.34\%$) while needing no DNS full field to place the sensors. Random placement remains feasible ($7.95\%$ KE) but less reliable; thus placement changes reliability more than feasibility. The final pipeline therefore adopts LES-derived QR-pivot placement as the deployable reference: it approaches the DNS oracle without needing a DNS full field for placement, and improves over random placement by $2.24$ KE percentage points.
...
\end{equation}
LES-derived placement improves both the mean (KE $5.71\%$ vs.\ $7.95\%$, $\Delta=2.24$ percentage points, Welch $z \approx 3.3$, $p<0.01$) and the reproducibility ($6.2\times$ tighter), so engineering deployment should prioritise placement optimisation over training repetition. Divergence ratio stays at $0.35 \pm 0.01\%$ across all five placement seeds: the divergence diagnostic (\S\ref{sec:sub_dns_div}) is placement-invariant within the random-uniform family.

\paragraph{Field alignment does not improve placement.}
The placement LES is an independent realisation, statistically converged yet pointwise-decorrelated from the DNS (\S\ref{sec:les_placement}). A natural expectation is that a more field-aligned LES (initialised from the DNS field at $t=0$, so that it tracks the DNS pointwise for roughly one eddy-turnover time, Fig.~\ref{fig:les_predictability}) would select better sensors. Table~\ref{tab:placement_dnsinit} refutes this. A DNS-initialised (dns-init) LES integrated only to $t=5$ ($\sim 2.5\,\tau$) gives a worse placement (KE $9.12\%$) than the statistically-converged random-initial-condition $T=50$ LES ($\sim 22\,\tau$, $5.90\%$) at the same training seed and configuration: DNS seeding does not help, it hurts. QR-pivoting selects positions from the leading POD modes, which require a statistically-converged trajectory; a DNS-seeded LES first sheds the small-scale energy it cannot sustain (an initial spin-down transient), so the short $t=5$ window captures that transient rather than the attractor. Pointwise field alignment at matched time is therefore irrelevant, indeed counterproductive, for placement, which supports selecting sensors from a statistically-converged, DNS-free $T=50$ LES. The dns-init run requires a DNS field to seed and is reported only as a diagnostic, not as a deployable option.

\begin{table}[!htbp]
    \centering
    \caption{Field-aligned versus statistically-converged sensor placement at $Re=10\,000$, $K=100$, B3, $1024$ collocation points, $20\,000$ iterations, single training seed $42$. The $t=5$ dns-init LES is initialised from the DNS field, so it tracks the DNS pointwise at early time, but it is a diagnostic (it needs a DNS field to seed), not a deployable option. KE is MAPE (percent); $\tau$ denotes the eddy-turnover time.}
...
Because the main pipeline relies on measured sensor time series rather than full-field supervision, moderate observation noise must not dominate reconstruction. Table~\ref{tab:noise_robustness} reports the response to per-channel Gaussian noise at $1$--$10\%$ of the sensor standard deviation, with the LES-derived layout and final $20\,000$-iteration, $1024$-collocation training setup fixed. The sweep uses $n=5$ seeds at each noise level and supports O3 by testing whether the trained recipe remains engineering-grade under controlled additive perturbations.

\begin{table}[!htbp]
    \centering
    \caption{Sensor-noise robustness of the LES-pivot pipeline under the final PI-CON protocol ($20\,000$ iterations, $1024$ collocation, $n=5$ seeds per noise level). Noise level is relative to the per-channel sensor standard deviation; $\Delta$~KE is measured against the clean final-protocol baseline in Table~\ref{tab:main_metrics}.}
...
PI-CON is evaluated at $Re=10^{6}$, two orders of magnitude above the primary test case.
...
Enstrophy rel-err ($42\%$) is higher than at $Re=10^{4}$, consistent with the wider gap between the sensor Nyquist ceiling ($k_\text{max}\approx8$ at $K=200$) and the $Re=10^{6}$ dissipation cut-off ($k\approx100$), rather than an architectural limitation.
...
Table~\ref{tab:inference_cost} reports the end-to-end pipeline cost. The encoder pass costs $70.7 \pm 3.8$\,ms once per trajectory; subsequent spatial queries cost $32\,\mu$s per grid point ($31\,030$ queries/s). Because the trunk evaluates any coordinate directly, the operator answers an arbitrary space--time query without re-integrating the solver from an initial condition: the query-anywhere property required by O1 (\S\ref{sec:research_questions}). Sparse monitoring (up to $\sim 3\,000$ query points) completes within $100$\,ms, whereas a full $128^2$ field query costs $527.8 \pm 17.1$\,ms, so single-shot real-time full-field reconstruction on CPU or MPS requires GPU or batched optimisation.

The one-time setup couples the placement-LES forward solve over the reconstruction window ($256^2$, $50\,000$ steps at $\Delta t = 10^{-4}$~s, $7.6$\,min) with operator training ($1.33$\,h), totalling $\approx 1.46$\,h against the $3.27$\,h required to solve the $1024^2$ DNS reference over the same window. The trained operator then reconstructs a full trajectory in $9.7$\,min and answers further space--time queries at negligible marginal cost, so the setup amortises across queries. This setup-versus-solve margin mixes hardware (GPU training against the CPU DNS solve) and is not the engineering case for the operator: forward simulation cannot reconstruct a field from sparse observations, and the DNS is unavailable on a new scene. The margin widens where a single high-fidelity solve is far more expensive (3D, multi-physics, or higher-Reynolds flows); establishing this remains an open follow-up.

\begin{table}[!htbp]
    \centering
    \caption{End-to-end cost of the PI-CON pipeline against the DNS reference. The one-time setup couples the placement-LES forward solve over the reconstruction window $t\in[0,5]$~s (the placement basis itself uses the longer integration of \S\ref{sec:les_placement}) with operator training; per-trajectory inference is then reused for arbitrary space--time queries. Wall-clock is single-host CPU (numpy) for the LES and DNS solves, a single GPU for training, and Apple M3 MPS for inference, so the setup-versus-DNS ratio mixes hardware. Times in milliseconds (ms), minutes (min), or hours (h) as indicated; throughput in queries per second (q/s).}
