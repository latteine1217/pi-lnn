...
\begin{table}[!htbp]
    ...
    KE MAPE & $5.71 \pm 0.12\%$ & $\mathbf{4.68 \pm 0.06\%}$ \\
    $u$ rel-$L_2$ & $\mathbf{13.65 \pm 0.06\%}$ & $15.34 \pm 0.06\%$ \\
    $v$ rel-$L_2$ & $\mathbf{17.52 \pm 0.10\%}$ & $18.10 \pm 0.03\%$ \\
    $\omega$ rel-$L_2$ & $\mathbf{41.77 \pm 0.12\%}$ & $42.41 \pm 0.12\%$ \\
...
The main row in Table~\ref{tab:main_metrics} uses no DNS full field for sensor placement or training supervision; sensor values are the DNS stand-in sampled only at the $K$ positions. Against the DNS-oracle reference, placement shows an integral/pointwise trade-off: the oracle has lower KE error ($4.68\%$ vs $5.71\%$), while LES-derived placement has lower pointwise velocity error ($u$ rel-$L_2$ $13.65\%$ vs $15.34\%$, $v$ rel-$L_2$ $17.52\%$ vs $18.10\%$). All evaluated strategies remain within the $10\%$ target: random placement gives KE $7.95\%$, LES-derived placement $5.71\%$, geometric space-filling $4.69\%$, and the non-deployable DNS oracle $4.68\%$. LES-derived QR-pivoting is therefore used as a recommended informed DNS-free configuration because it improves over random placement by $2.24$ KE percentage points in this setting, not because it is universally optimal; space-filling gives lower KE error, whereas LES-derived placement gives lower pointwise error (\S\ref{sec:placement_analysis}).

The divergence ratio of $0.39\%$ confirms the continuity constraint is active, matching the finite-difference floor of a DNS field band-limited to the reconstruction's resolved bandwidth (\S\ref{sec:sub_dns_div}; Fig.~\ref{fig:main_trajectories}, second panel). The energy-dominant low-wavenumber band (sensor-count sampling band edge $k_{\max}^{\rm sensor}\approx 5.64$, carrying $\approx 99\%$ of kinetic energy at $t=5$; \S\ref{subsec:nyquist_scaling}) recovers to single-digit relative error (Fig.~\ref{fig:band_err}), within the $10\%$ engineering target of O1.
...
\begin{table}[!htbp]
    ...
    \midrule
    \textbf{mean$\pm$std} & $\mathbf{5.71 {\pm} 0.12}$ & $\mathbf{13.65 {\pm} 0.06}$ & $\mathbf{17.52 {\pm} 0.10}$ & $\mathbf{41.77 {\pm} 0.12}$ & $\mathbf{0.39 {\pm} 0.006}$ & $\mathbf{0.991 {\pm} 0.005}$ \\
    \bottomrule
    \end{tabularx}
\end{table}

\paragraph{Reproducibility.}
The seed-to-seed KE standard deviation is $0.12$ percentage points ($2.1\%$ relative to the mean); all five seeds fall within a $0.32$ percentage-point window ($5.59$--$5.90\%$). Pointwise $u$-$L_2$ and $v$-$L_2$ are tighter still ($\sigma=0.06\%$ and $0.10\%$, both below $1\%$ relative scatter). The divergence ratio is reproducible to two decimal places ($0.39 \pm 0.006\%$), confirming it is a property of the architecture--AL recipe rather than any individual seed (\S\ref{sec:sub_dns_div}).

\paragraph{Architectural significance.}
The architectural advantage holds at budget parity. With all four ablation cells trained at the matched $1\,024$-collocation $20\,000$-iteration $n=5$-seed regime, a Welch $t$-test between B0 (Vanilla DeepONet, $8.23 \pm 0.22\%$) and B3 (Full PI-CON, $5.71 \pm 0.12\%$) gives a KE gap of $-2.53$ percentage points ($t=22.9$, $p=3.0\times10^{-7}$, Cohen's $d=14.5$): the gain is significant and not an artefact of seed count or iteration budget. The full $2\times2$ component decomposition appears with the ablation matrix (\S\ref{sec:baseline_comparison}).
...
\begin{figure}[!htbp]
    ...
    \caption{Radial energy spectrum at $t=5$ for the representative single seed (seed 42). Horizontal axis: wavenumber $k$ (1/m), log scale; vertical axis: radial energy spectrum $E(k)$ (m$^3$/s$^2$), log scale. DNS (solid black) and PI-CON prediction (dashed blue). Vertical dash-dotted line marks the forcing wavenumber $k_f = 2$; vertical dashed green line marks the sensor-count sampling band edge $k_{\max}^{\rm sensor}\approx 5.64$.}
    \label{fig:spectrum}
\end{figure}

Figure~\ref{fig:spectrum} shows the radial energy spectrum at $t=5$. PI-CON matches the DNS spectrum within the sensor sampling band $k\le k_{\max}^{\rm sensor}\approx 5.64$ (carrying $\approx 99\%$ of the total energy at $t=5$); for $k\ge 16$ the prediction drops to a near-constant floor of $\approx 10^{-7}$, whereas the DNS continues a Kolmogorov-like cascade up to $k\sim 64$.
...
\begin{table}[!htbp]
    ...
    B3 Full PI-CON (Ours)   & $\mathbf{13.65 {\pm} 0.06}$ & $\mathbf{17.52 {\pm} 0.10}$ & $\mathbf{41.77 {\pm} 0.12}$ & $\mathbf{5.71 {\pm} 0.12}$ & $3.14$\,M \\
    \bottomrule
    \end{tabularx}
\end{table}

\paragraph{Architectural ranking under the main pipeline.}
With all four cells trained at the matched $20\,000$-iteration, $n=5$-seed budget, the KE ordering is B3 ($5.71 \pm 0.12\%$) $\prec$ B2 ($7.03 \pm 0.14\%$) $\prec$ B0 ($8.23 \pm 0.22\%$) $\prec$ B1 ($9.23 \pm 0.51\%$). Welch $t$-tests confirm every gap to B3 is significant: $-2.53$ percentage points versus B0 ($t=22.9$, $p=3.0\times10^{-7}$), $-3.52$ versus B1 ($t=15.1$, $p=5.5\times10^{-5}$), and $-1.32$ versus B2 ($t=16.3$, $p=2.4\times10^{-7}$). Decomposing the $2\times2$ on the absolute KE scale about the B0 reference cell (where the decomposition is additive) separates a cross-attention main effect of $-1.21$ percentage points, a CfC main effect of $+0.99$, and a CfC$\times$cross-attention interaction of $-2.31$, summing to the $-2.53$ B3$-$B0 total. Cross-attention is the dominant lever. CfC raises KE error on its own (mean-pool aggregation in B1 erodes the per-query specificity that physics-residual sampling supplies) but carries a large negative interaction, so it lowers error only in combination with the cross-attention readout.

\section{Sensing-Configuration Study}
\label{sec:sensing_study}

Three sensing-configuration levers govern reconstruction quality: sensor count, placement, and measurement noise. This section quantifies the sensor count and its sampling band edge, then reports placement and noise robustness.

\subsection{K-Scaling Against the Sampling Band Edge}
\label{subsec:nyquist_scaling}

The most direct test of the information-limited interpretation varies sensor count while keeping the LES-derived placement principle fixed. The prediction under test is the sampling-density band edge $k_{\max}^{\rm sensor} = \sqrt{K/\pi}$ derived in \S\ref{sec:underdetermined}, a scale for the reconstructable band rather than a hard cutoff: it is a necessary condition on the sample count, and it charges one scalar sample to each sensor, whereas the two measured velocity components $(u,v)$ of a divergence-free field place the strict linear-observability limit higher (for $K=100$ the sensing matrix stays full-rank up to $k\lesssim 8$, Appendix~\ref{app:info_ceiling_diagnostics}). Raising $K$ from $100$ to $200$ and $400$ moves the band edge from $\approx 5.64$ to $7.98$ and $11.28$.
...
\begin{figure}[!htbp]
    ...
    \caption{DNS energy available within the sensor-count sampling band edge $\sqrt{K/\pi}$ for $K$ point velocity sensors ($Re=10^4$, $N=256$, $t=5$~s). (a)~Radial DNS energy spectrum $E(k)/E_{\rm tot}$ (dimensionless) versus wavenumber $k$ (1/m), with the $k^{-3}$ enstrophy-cascade reference (dotted; the forward-cascade slope for $k>k_f$) and sampling band edges $k_{\max}^{\rm sensor}=\sqrt{K/\pi}$ (vertical dashed, colour-coded by $K$). (b)~Cumulative DNS energy fraction $F_{\rm DNS}(k)$ (dimensionless) versus $k$ (1/m); markers locate $F_{\rm DNS}(k_{\max}^{\rm sensor})$ for $K\in\{100,200,400\}$.}
    \label{fig:nyquist_recoverability}
\end{figure}

Table~\ref{tab:k_scaling_nyquist} reports the empirical reconstruction following this predicted bandwidth expansion.

\begin{table}[!htbp]
    \centering
    \caption{Sensor-count scaling against the sampling band edge $\sqrt{K/\pi}$. LES-derived QR-pivot sensors, seed 42, $20\,000$ iterations; metrics at $t=5$. The $K=400$ run uses $512$ collocation points (memory budget).}
    ...
    \midrule
    Band edge $k_{\max}^{\rm sensor}$ & 5.64 & 7.98 & 11.28 \\
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption{Radial energy spectrum $E(k)$ (m$^3$/s$^2$) versus wavenumber $k$ (1/m) at $t=5$ for three sensor budgets. Each panel: DNS (solid black), PI-CON (dashed blue), $k^{-3}$ enstrophy-cascade reference (dotted grey; the forward-cascade slope for $k>k_f$), forcing $k_f = 2$ (dash-dotted black), sensor sampling band edge $k_{\max}^{\rm sensor}=\sqrt{K/\pi}$ (dashed green). LES-derived QR-pivot, seed $42$, $20\,000$ iterations.}
    \label{fig:k_scaling_spectra}
\end{figure}

The agreement is qualitative but clear: at $K=100$ the $\sqrt{K/\pi}$ scale sits near the low/mid-band boundary, $K=200$ pushes it close to $k\approx 8$, and $K=400$ reaches into the inertial-range content (Figure~\ref{fig:k_scaling_spectra}). KE error drops by $70\%$ between $K=100$ and $K=400$ ($5.90\%\to1.76\%$; the $K=100$ value is the seed-42 single run, whose $n=5$ mean is $5.71\%$, Table~\ref{tab:multiseed}). The observed KE ratios relative to $K=100$ are $0.42$ at $K=200$ and $0.30$ at $K=400$. The band-edge relation $k_{\max}^{\rm sensor}\propto\sqrt{K}$, under which the unresolved energy above the cutoff scales as $k_{\max}^{-2}\propto 1/K$, predicts ratios of $100/K = 0.50$ and $0.25$ respectively, placing the measured curve within $20\%$ of this scaling estimate. Because the $K=400$ run uses fewer collocation points, the three-point curve should not be read as a strict fit; it shows that higher fidelity comes from sensor bandwidth expansion, not from architecture search.


\subsection{Information-Ceiling Summary}
\label{sec:info_limit_summary}

The $K=100$ result should be read against the sensor information ceiling rather than as an unconstrained full-field regression problem. A wavelet sparsity diagnostic, a sensing-matrix rank calculation, and a spectral-truncation bound point to the same interpretation: the low-wavenumber band is observable enough to support engineering KE and mean-flow monitoring, while mid- and high-wavenumber content remains underdetermined at this sensor budget. The B3 multi-seed KE error ($5.71\%$) maps to an effective spectral cutoff $k_{\rm cut}\approx 4.7$ and the $k^2$-weighted vorticity error to $k_{\rm cut}\approx 7.8$ (Appendix~\ref{app:info_ceiling_diagnostics}); the two bracket the sensor sampling band edge $k_{\max}^{\rm sensor}\approx 5.64$, as the sensor-count criterion of \S\ref{sec:research_questions} requires. Closing the remaining mid/high-band error therefore requires more sensors or extra modalities, not another architecture-side loss term.

\paragraph{Relation to the synchronisation threshold of continuous data assimilation.}
The same ceiling can be stated in the language of continuous data assimilation, and stating it there fixes what this work does and does not claim. Nudging a solver toward observations recovers the full state once the observed fraction exceeds a critical density; in three-dimensional homogeneous isotropic turbulence that threshold is $\varphi_c \approx 0.2$~\cite{ClarkDiLeoni2020PRX}, whereas the $K=100$ probes used here cover $0.15\%$ of the domain. The present setting therefore sits far outside the regime in which any observation-driven scheme, learned or otherwise, would be expected to synchronise the full field, and PI-CON claims no such synchronisation. What the sensor budget supports is band-limited: low-wavenumber content up to $k_{\rm cut}\approx 4.7$ is recovered to engineering accuracy, while mid- and high-wavenumber content stays underdetermined. Full synchronisation and band-limited reconstruction are distinct targets, and the results above speak only to the second. On this reading the bounded mid/high-band error follows from the sensor budget rather than from a shortfall of the architecture.
...
Placement strategy contributes a variance source larger than training stochasticity at fixed $K=100$. Table~\ref{tab:placement_strategy_new} summarises the final-protocol placement comparison. DNS-oracle, LES-derived, and geometric space-filling placements are each trained over the same $n=5$ random seeds at $20\,000$ iterations; random placement is repeated over $n=5$ placement seeds with the training seed fixed to isolate placement variance.

\begin{table}[!htbp]
    \centering
    \caption{Final-protocol placement comparison at $Re=10\,000$, $K=100$, B3, $1024$ collocation points, $20\,000$ iterations. DNS-oracle, LES-derived, and geometric space-filling placements report training-seed mean $\pm$ standard deviation. Random placement reports placement-seed mean $\pm$ standard deviation with training seed fixed at $42$. In all rows the sensor \emph{values} are read from the DNS field that stands in for the real-world flow (at the $K$ sensor positions only); the second column marks only whether \emph{placement} additionally requires the DNS \emph{full field}.}
    ...
    Geometric space-filling & No & $\mathbf{4.69 \pm 0.10}$ & $14.73 \pm 0.19$ & zero-simulation lower-KE reference \\
    LES-derived QR-pivot & No & $5.71 \pm 0.12$ & $\mathbf{13.65 \pm 0.06}$ & recommended informed placement \\
    Random uniform & No & $7.95 \pm 0.76$ & $17.20 \pm 1.42$ & no-placement-effort reference \\
    \bottomrule
    \end{tabularx}
\end{table}

\paragraph{Strategy comparison.}
All four strategies stay below the $10\%$ KE engineering target. The DNS oracle and geometric space-filling give the lowest integral KE errors ($4.68\%$ and $4.69\%$), whereas LES-derived placement gives the lowest pointwise velocity error while needing no DNS full field to place the sensors. Random placement remains feasible ($7.95\%$ KE) but less reliable. These results support recommending LES-derived QR-pivoting as an informed DNS-free option over random placement in the evaluated setting; they do not establish a universally optimal strategy because the ranking depends on the metric and placement cost.
...
\begin{table}[!htbp]
    ...
    \midrule
    \textbf{mean$\pm$std} & $\mathbf{7.95 {\pm} 0.76}$ & $17.20 {\pm} 1.42$ & $21.62 {\pm} 2.07$ & $\mathbf{0.35 {\pm} 0.01}$ & $0.973 {\pm} 0.024$ \\
    \bottomrule
    \end{tabularx}
    \\[2pt]
...
The two five-seed estimates yield $\sigma_{\rm training}=0.12\%$ (LES $T=50$, Table~\ref{tab:multiseed}) and $\sigma_{\rm placement}=0.76\%$ (random uniform, Table~\ref{tab:placement_variance}); placement variance dominates training stochasticity by
\begin{equation}
    \sigma_{\rm placement}/\sigma_{\rm training} \;\approx\; 6.4\times.
    \label{eq:placement_dominance}
\end{equation}
The mean comparison shows that LES-derived placement improves KE over random placement ($5.71\%$ vs.\ $7.95\%$, $\Delta=2.24$ percentage points, Welch $z \approx 3.3$, $p<0.01$). The ratio in \eqref{eq:placement_dominance} separately shows that changing random sensor locations introduces more variability than retraining at one fixed LES-derived placement; it does not establish that LES-derived QR-pivoting is $6.4\times$ more reproducible than every alternative. Together, the two observations support using an informed placement strategy rather than relying on an arbitrary random layout. Divergence ratio stays at $0.35 \pm 0.01\%$ across all five placement seeds: the divergence diagnostic (\S\ref{sec:sub_dns_div}) is placement-invariant within the random-uniform family.
...
\begin{table}[!htbp]
    ...
    \midrule
    Clean & $5.71 {\pm} 0.12$ & -- & $13.65 {\pm} 0.06$ & $17.52 {\pm} 0.10$ & $0.39 {\pm} 0.006$ \\
...
Results are in Table~\ref{tab:re1e6}. KE MAPE attains $6.10\%$, close to the $Re=10^{4}$ baseline level ($5.71 \pm 0.12\%$, Table~\ref{tab:main_metrics}) but not directly comparable as a seed-statistics claim because the $Re=10^{6}$ run is single-seed and uses a retuned sensor budget, width, and step count. This is a feasibility case at higher Reynolds number, not a like-for-like generalisation or recipe-transfer claim.
...
Enstrophy rel-err ($42\%$) is higher than at $Re=10^{4}$, consistent with the wider gap between the sensor-count sampling band edge ($k_\text{max}\approx8$ at $K=200$) and the $Re=10^{6}$ dissipation cut-off ($k\approx100$), rather than an architectural limitation.
...
\paragraph{Sensor budget across Reynolds number.}
The $Re=10^{6}$ case above retunes sensor count and capacity simultaneously, so it does not isolate how the required budget itself varies with $Re$. A protocol-matched sweep separates the two. Five Reynolds numbers, $Re \in \{10^{2},\,5\times10^{2},\,10^{3},\,10^{4},\,10^{6}\}$, are each reconstructed at $K \in \{10,\,50,\,100\}$ under one fixed configuration: coverage-optimal farthest-point placement, an observation window matched to five eddy turnover times, and the architecture, optimiser, collocation count, and seed of the $Re=10^{4}$ baseline. Every reference field is a resolved DNS ($k_{\max}/k_d \ge 2.0$ on the simulation grid). Holding placement, window, capacity, and seed fixed leaves sensor count and Reynolds number as the only varied factors.

All reconstructions are evaluated on a common $128^{2}$ grid and on time samples aligned to $101 \pm 4$ frames, so that neither the metric resolution nor the temporal sampling density varies with Reynolds number.

Figure~\ref{fig:crossre_sensor_budget}(a) reports kinetic energy. The budget needed to reach the $10\%$ engineering threshold rises with Reynolds number: $K \le 10$ at $Re=10^{2}$, $K \in (10, 50]$ at $Re=5\times10^{2}$ and $10^{3}$, $K \in (50, 100]$ at $Re=10^{4}$, and $K > 100$ at $Re=10^{6}$. The ordering follows the sampling-band argument of \S\ref{subsec:nyquist_scaling}: what a fixed budget must cover is the spectral support of the field, and the saturated bandwidth of these flows widens with $Re$. Reading the required budget against the band edge gives $K \gtrsim \pi n_{99}^{2}/2$, where $n_{99}$ is the integer wavenumber enclosing $99\%$ of the energy. In a decaying flow $n_{99}$ falls through the window, so the definition must be fixed before the estimate means anything: the value at the first frame is $8$ at every Reynolds number, inherited from the common band-limited initial condition, and carries no discriminating power, while an energy-weighted average is not monotone in $Re$. Taking the window mean, the estimate is $7$, $17$, $25$, $100$, and $156$ sensors for $Re = 10^{2}$ to $10^{6}$, against measured budgets of $10$, $50$, $50$, $100$, and beyond $100$. The estimate reproduces the ordering and lands on the measured value at the two highest Reynolds numbers, but understates the mid-range by a factor of two to three. It is a scale for the required budget, not a quantitative predictor of it.

\paragraph{The budget depends on which quantity is required.}
Kinetic energy is an energy-weighted integral dominated by the largest scales, so a budget sufficient for it need not suffice for quantities weighted towards high wavenumber. Panel (b) repeats the sweep for vorticity, and the thresholds move: at $Re=5\times10^{2}$ the requirement rises from $K=50$ to $K=100$, and at $Re=10^{3}$ from $K=50$ to beyond the tested range. Velocity sits between the two, with $Re=10^{3}$ requiring $K=100$ against $K=50$ for kinetic energy. Sensor budgets quoted on a kinetic-energy criterion are therefore lower bounds on what a vorticity-accurate reconstruction demands, by a factor of two or more over the tested range. Table~\ref{tab:crossre_kstar} lists the thresholds per quantity.

\paragraph{Where the operator gains over classical interpolation.}
Reconstructing the same cells from the same sensors with three DNS-free classical methods separates a hard sensing regime from a weak reconstructor. Radial basis interpolation, inverse-distance weighting, and least squares onto the modes inside the sampling band edge see the identical sensor values and no reference field, and are scored with the same formulas on the same grid. Across the twelve turbulent cells, band-limited least squares attains the lower kinetic-energy error in eleven, while the operator attains the lower velocity error in all twelve and the lower vorticity error in eleven. At $Re=10^{3}$ and $K=100$ the two sit at $1.37\%$ against $2.59\%$ on kinetic energy and at $12.46\%$ against $4.99\%$ on velocity. The gain the operator delivers is pointwise, and it is bought at the cost of the energy integral; a criterion read on kinetic energy alone ranks a linear fit above it. This reinforces the per-quantity reading of the budget above: kinetic energy separates sensing regimes but does not separate reconstruction methods.

At $K=10$ the ordering among methods compresses: every method lands between $28\%$ and $90\%$ on kinetic energy, and the least-squares fit degrades past $100\%$ on velocity because its band edge $\lfloor\sqrt{K/\pi}\rfloor = 1$ falls below the forcing wavenumber $k_f = 2$, leaving the injected mode outside its basis. The residual error at that budget is therefore a property of the sensing configuration rather than of any one reconstructor. Table~\ref{tab:crossre_baselines} lists the comparison.

\paragraph{Reference points from a modal and a sequential estimator.}
Two further reconstructions place the comparison against methods that carry a model of the flow. A gappy proper-orthogonal-decomposition fit uses a basis built from the placement LES rather than from the reference, keeping it free of the DNS; its coefficients are fitted to the same sensors. At $Re=10^{3}$ and $K=100$ it reaches $2.55\%$ on kinetic energy and $17.55\%$ on velocity. The projection of the reference onto that same basis --- the error a perfect coefficient fit would still leave --- is $12.00\%$, rising to $32.91\%$ at $Re=10^{4}$ and $58.05\%$ at $Re=10^{6}$. The operator's velocity error sits below that projection floor at all three Reynolds numbers, so its accuracy is not reachable by any reconstruction confined to the LES subspace, independent of how the coefficients are obtained.

An ensemble Kalman filter~\cite{Houtekamer2001Sequential} supplies the second reference, and is the estimator that uses the same information the operator is given: the sensors and the governing equations, with no reference field. Its forecast model is a pseudo-spectral vorticity-form integrator of the same governing equations, which drifts from the reference by $0.005\%$ over one observation interval when started from a reference snapshot. Following the settings reported for two-dimensional turbulence at comparable observation sparsity~\cite{Hasegawa2023LBMLETKF} --- $96$ members and an observation error of $10\%$ of the sensor root-mean-square --- the filter reaches $2.53\%$ on kinetic energy and $26.29\%$ on velocity at $Re=10^{3}$ and $K=100$, discarding the first half of the window as spin-up. The pattern of the interpolation baselines recurs: the energy integral is matched while the pointwise field is not, which is the behaviour reported for this filter class, where the spectral amplitude is retained and the phase error grows as observations thin~\cite{Hasegawa2023LBMLETKF}. This single configuration is a reference point rather than a tuned comparison; the filter had not converged when the window ended, and a localised formulation~\cite{Hunt2007LETKF, Hamill2001Localization} would be the next step for a like-for-like study.

\begin{table}[!htbp]
    \centering
    \caption{Relative error (dimensionless, reported in percent) at $K=100$ for the proposed operator and three DNS-free classical reconstructions, on a common $128^{2}$ grid. RBF denotes radial basis interpolation, IDW inverse-distance weighting, and LSQ least squares onto modes inside the sampling band edge. The $Re=10^{2}$ row is the laminar end member.}
    \label{tab:crossre_baselines}
    \small
    \begin{tabular}{llcccc}
    \toprule
    \textbf{Quantity} & \textbf{$Re$} & \textbf{PI-CON} & RBF & IDW & LSQ \\
    \midrule
    \multirow{5}{*}{Kinetic energy}
      & $10^{2}$        & 0.73  & 3.59  & 72.83 & \textbf{0.02} \\
      & $5\times10^{2}$ & 1.26  & 4.12  & 62.46 & \textbf{0.43} \\
      & $10^{3}$        & 2.59  & 6.32  & 64.34 & \textbf{1.37} \\
      & $10^{4}$        & 4.56  & 9.97  & 67.18 & \textbf{3.10} \\
      & $10^{6}$        & 13.50 & 11.90 & 70.18 & \textbf{3.88} \\
    \midrule
    \multirow{5}{*}{Velocity $u$}
      & $10^{2}$        & 1.26  & 2.90  & 54.32 & \textbf{1.06} \\
      & $5\times10^{2}$ & \textbf{2.50}  & 6.80  & 51.04 & 5.35 \\
      & $10^{3}$        & \textbf{4.99}  & 11.86 & 52.09 & 12.46 \\
      & $10^{4}$        & \textbf{14.46} & 25.97 & 51.33 & 32.16 \\
      & $10^{6}$        & \textbf{25.51} & 31.44 & 57.42 & 39.45 \\
    \bottomrule
    \end{tabular}
\end{table}

Three boundaries limit the reading. Each point is single-seed and uses one placement realisation, so the curves carry neither seed nor placement statistics, and the threshold crossings are bracketed by the tested grid rather than resolved within it. The $Re=10^{2}$ case relaxes to the steady laminar Kolmogorov solution over the matched window; its reconstruction is not representative of the turbulent regime and is retained as the laminar end member, and its transverse velocity error is not reported because the reference $v$ vanishes there. At $K=10$ every turbulent case exceeds $40\%$ error, and differences within that range are not interpreted.

\begin{table}[!htbp]
    \centering
    \caption{Smallest tested sensor count $K$ (dimensionless) reaching $10\%$ relative error, by reconstructed quantity. Entries are bracketed by the tested grid $K \in \{10, 50, 100\}$. The $Re=10^{2}$ row is the laminar end member; its $v$ entry is omitted because the reference transverse velocity vanishes.}
    \label{tab:crossre_kstar}
    \small
    \begin{tabular}{lccc}
    \toprule
    \textbf{Reynolds number} & \textbf{Kinetic energy} & \textbf{Velocity $u$} & \textbf{Vorticity} \\
    \midrule
    $10^{2}$ (laminar)  & $10$    & $10$    & $10$    \\
    $5\times10^{2}$     & $50$    & $50$    & $100$   \\
    $10^{3}$            & $50$    & $100$   & $>100$  \\
    $10^{4}$            & $100$   & $>100$  & $>100$  \\
    $10^{6}$            & $>100$  & $>100$  & $>100$  \\
    \bottomrule
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/results/crossre_sensor_budget.pdf}
    \caption{Relative error (dimensionless, reported in percent) against sensor count $K$ (dimensionless) at five Reynolds numbers, for (a) kinetic energy and (b) vorticity. All runs use farthest-point placement, an observation window of five eddy turnover times, $d_\text{model}=256$, $20\,000$ iterations, $1\,024$ collocation points, and seed 42; reference fields are resolved DNS. Both panels are evaluated on a common $128^{2}$ grid over $101 \pm 4$ time samples spanning the observation window. The dotted line marks the $10\%$ threshold. Single seed and one placement realisation per point.}
    \label{fig:crossre_sensor_budget}
\end{figure}


\paragraph{Diagnostic boundaries.}
Several diagnostic findings define the boundary of the final recipe without changing the main result. The Kolmogorov forcing parameters are not recovered from $K=100$ sensors under the tested setup (near-zero flat-region initialisation): accurate velocity reconstruction did not certify recovery of $A$ or $k_f$, though structural identifiability is not established. Extending Augmented-Lagrangian enforcement from continuity to the Navier--Stokes momentum residual is not promoted as a main-recipe improvement: the final-protocol rerun gives mixed single-seed results, so the final pipeline keeps the $n=5$-validated continuity-only AL recipe and leaves momentum residuals to GradNorm weighting. A further single-seed check exposes the full model and the vanilla-branch ablation to irregular sensor dropout at matched training budget and parameter count, and separates the error at retained and discarded query times (Appendix~\ref{sec:intermittent_sensors}). These functional-parameter and recipe diagnostics are reported in Appendix~\ref{app:diagnostic_ablation_forward_cfd} rather than used as Chapter~\ref{ch:results} evidence.
