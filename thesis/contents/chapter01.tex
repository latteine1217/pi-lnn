...
A second anchor bounds not the exactness of recovery but its bandwidth, and it comes from sampling theory rather than from sparsity. Its ancestor is the Nyquist--Shannon sampling theorem~\cite{Shannon1949}, which sets how densely a band-limited signal must be sampled to be recoverable: one sample for every mode to be resolved. That statement is one-dimensional and phrased for a uniform grid, so it does not transfer directly to sensors scattered over a plane. The result that does transfer is Landau's necessary density condition~\cite{Landau1967}, which restates the same requirement in terms of sample \emph{density} instead of sample spacing and holds for arbitrary sample geometries in any number of dimensions. Read for $K$ sensors on the unit-area domain and a spectrum occupying the disk $|\mathbf{k}| \le k_{\max}$, it places the admissible band edge at
\begin{equation}
    \label{eq:sensor_nyquist}
    k_{\max}^{\rm sensor} \;=\; \sqrt{K/\pi},
\end{equation}
the two-dimensional counterpart of the one-dimensional Nyquist limit, giving $k_{\max}^{\rm sensor} \approx 5.64$ at $K=100$.

Two qualifications on \eqref{eq:sensor_nyquist} carry through this work. It is a necessary condition on the sample count and not a guarantee of recovery: conditioning and measurement noise shrink the band that is recoverable in practice, so the scale is an upper reference rather than an attainable cutoff. It is also a conservative count, because it charges one scalar sample to each sensor, whereas a sensor here reports both velocity components while a divergence-free planar field carries a single scalar degree of freedom per mode. The band edge of~\eqref{eq:sensor_nyquist} therefore sits below the wavenumber at which the measurements cease to determine the field, and the two are kept distinct throughout: \eqref{eq:sensor_nyquist} names the scale against which the reconstructed bandwidth is read, while the observability edge is established numerically from the sensing matrix rather than from a second counting formula. Sampling-density reasoning of this kind is established in fluid mechanics on both the numerical and the experimental side --- it fixes the resolved wavenumber range of a spectral simulation~\cite{MoinMahesh1998} and the cut-off wavenumber of PIV interrogation~\cite{Foucaut2004} --- while sparse-sensor reconstruction has instead been analysed through rank and compressed-sensing arguments~\cite{Willcox2006, Bourguignon2014}. Both readings are used here: \eqref{eq:cs_bound} for exact recovery, \eqref{eq:sensor_nyquist} for the band edge.
...
The reconstructable wavenumber band must be governed by the sensor count rather than the architecture: $K$ point sensors set the sampling-density band edge $k_{\max}^{\rm sensor}$ of~\eqref{eq:sensor_nyquist}, with the effective cutoff limited by sensor information and conditioning. The criterion is: (i) the effective cutoffs $k_{\rm cut}$ implied by the kinetic-energy and vorticity errors bracket $k_{\max}^{\rm sensor} \approx 5.64$ at $K = 100$, the two differing because one sharp-cutoff model is fitted in turn to an energy-weighted and a $k^2$-weighted error, with the mid-band ($5 < k \le 16$) and high-band ($k > 16$) errors saturating at the compressed-sensing ceiling of~\eqref{eq:cs_bound}; and (ii) sensor scaling $K \in \{100, 200, 400\}$ shifts the reconstructed bandwidth in agreement with the $\sqrt{K/\pi}$ trend.

\paragraph{O3: Separating reliability from feasibility across placement and noise.}
Reconstruction quality must depend on how sensors are placed and how noisy they are, while staying within the engineering target across reasonable choices. The criterion is: (i) the four placement strategies (a DNS-pivot oracle, DNS-free LES-derived QR-pivoting, DNS-free geometric space-filling, and random placement) all remain within the target, with the placement-induced standard deviation exceeding the training-seed standard deviation by $\ge 3\times$; (ii) LES-derived QR-pivoting improves reconstruction over random placement sufficiently to justify its recommendation as an informed DNS-free option, without requiring it to dominate every metric or alternative strategy; and (iii) additive measurement noise up to $10\%$ of the sensor standard deviation keeps the reconstruction within the target in the noise-stress setting.
