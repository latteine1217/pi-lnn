...
In the LES-derived placement strategy, a low-cost LES on a new scene supplies the POD basis for QR-pivot sensor positions; the physical flow then supplies sensor values. In this controlled study, the DNS substitutes for that physical flow only at the $K$ sensor locations and for offline scoring, not for placement or training. The LES and DNS are independent realisations with the same $Re$, domain, and forcing but different random initial fields, so the LES is used as a statistically converged placement tool rather than as a pointwise trajectory to match. Statistics set the LES horizon, while the DNS reconstruction target remains the shorter window $t\in[0,5]$ (Tables~\ref{tab:dns_params} and~\ref{tab:les_params}).
...
\end{align}
\end{subequations}
on the same domain, viscosity, and forcing as the DNS. Equations~\eqref{eq:filtered_cont}--\eqref{eq:filtered_mom} differ from the DNS only through filtering and the SGS stress term. The high-wavenumber SGS drain is a spectral hyperviscosity term, used alone in the reference LES; the low-fidelity variant additionally applies a Bardina scale-similarity term~\cite{Bardina1980, Sagaut2006},
\begin{equation}
...
A same-initial-condition diagnostic bounds the pointwise lifetime of even a reference-grade LES. Initialising the LES from the DNS field and integrating to $t=5$~s keeps velocity relative-$L^2$ below $5\%$ for $0.59\,t_{\rm eddy}$ and below $20\%$ for $1.10\,t_{\rm eddy}$; by $t\approx 4$~s the full-field error peaks near $92\%$ (Fig.~\ref{fig:les_predictability}). Velocity correlation remains above $0.9$ until $\approx 1.5\,t_{\rm eddy}$, while vorticity correlation falls to $\approx 0.1$ by $t\approx 4$~s. Thus even the most favourable matched-IC pair loses pointwise agreement within about one-and-a-half eddy-turnover times; deployable placement must rely on statistically converged POD structure, not matched-time fields.
...
Over $k\in[3,30]$ the same-initial-condition LES and DNS spectra agree (slopes $-4.56$ and $-4.64$; Fig.~\ref{fig:les_vort_compare}b), while LES energy falls below DNS for $k\gtrsim 30$ as hyperviscosity drains small scales. This high-wavenumber offset bounds high-band reconstruction but not QR-pivot placement, which depends on leading POD modes. The placement LES is integrated to $T_{\rm end}=50$ ($26.5\,t_{\rm eddy}$) for that statistical convergence.
