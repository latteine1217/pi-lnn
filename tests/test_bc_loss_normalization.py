...
def test_training_imports_required_closures_for_bc_loss():
    ...
    import pi_con.training as t
    # BC loss 使用的兩個 closure factory；training.py 必須 import 兩者。
    assert hasattr(t, "make_picon_model_fn"), (
        "training.py 必須 import make_picon_model_fn（BC loss 在 [training.py:670] 用到）"
    )
    assert hasattr(t, "make_picon_model_fn_uvp"), (
        "training.py 必須 import make_picon_model_fn_uvp（physics path 用到）"
    )
