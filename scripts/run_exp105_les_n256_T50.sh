...
#
# What:
#   EXP-105 完整 pipeline：用 LES-informed QR-pivot sensor 訓練 B3 model 10k steps
...
#   4. 訓練完接 evaluator，metrics 寫進 logs/exp_105/metrics.tsv
...
LOGS_DIR="$REPO_ROOT/logs/exp_105"
mkdir -p "$LOGS_DIR"
...
CONFIG="configs/exp_105_b3_les_n256_T50_qrpivot.toml"
ARTIFACT_DIR="artifacts/kolmogorov/deeponet-cfc-re10000-exp105-b3-les-n256-T50-seed2"
FINAL_PT="${ARTIFACT_DIR}/lnn_kolmogorov_final.pt"
EVAL_DIR="${ARTIFACT_DIR}/deeponet-cfc-eval"
SUMMARY="${EVAL_DIR}/summary.json"
TRAIN_LOG="${LOGS_DIR}/exp_105_train.log"
EVAL_LOG="${LOGS_DIR}/exp_105_eval.log"

main () {
  echo "======================================================================"
  echo "[EXP-105] LES-informed QR-pivot, B3 seed=2  $(date '+%Y-%m-%d %H:%M:%S')"
  echo "  config:    ${CONFIG}"
  echo "  artifact:  ${ARTIFACT_DIR}"
  echo "======================================================================"

  # ─── 1. Sanity check sensor files exist ────────────────
  local sensor_json="data/kolmogorov_sensors/re10000/sensors_qrpivot_K100_N256_t0-5_si100_les_n256_T50standalone.json"
  local sensor_npz="data/kolmogorov_sensors/re10000/sensors_qrpivot_K100_N256_t0-5_si100_les_n256_T50standalone_dns_values.npz"
  if [ ! -f "$sensor_json" ] || [ ! -f "$sensor_npz" ]; then
    echo "[EXP-105] ERROR: sensor file 不存在；請先跑 generate_sensors_qrpivot_from_les.py" >&2
    echo "  expected: $sensor_json" >&2
    echo "  expected: $sensor_npz" >&2
    exit 1
  fi
  echo "[EXP-105] sensor files OK"
  ...
  printf "EXP-105\t%d\t%d\t%d\t%s\n" "$t0" "$t1" "$dur" "$human" >> "$TIMING_TSV"
  echo "[EXP-105] training wall-time: ${human}"

  if [ ! -f "$FINAL_PT" ]; then
    echo "[EXP-105] ERROR: ${FINAL_PT} 不存在；訓練失敗" >&2
    exit 1
  fi
  ...
  if [ -f "$SUMMARY" ]; then
    ...
    printf "EXP-105\t%s\t%s\t%s\t%s\t%s\t%s\n" \
      "$u_l2" "$v_l2" "$omega_l2" "$ke" "$ek_ratio" "$div_l2" >> "$METRICS_TSV"
    echo "[EXP-105] metrics → ${METRICS_TSV}"
  else
    echo "[EXP-105] WARN: ${SUMMARY} 不存在；metrics 未紀錄" >&2
  fi

  echo ""
  echo "[EXP-105] === DONE at $(date '+%Y-%m-%d %H:%M:%S') ==="
  echo ""
  echo "=== Comparison ==="
  echo "baseline EXP-094 (DNS-pivot, seed=2):  KE ≈ 9.4%   (training placement)"
  echo "random pseed=1 (alt-eval, no retrain): KE ≈ 59.4%"
  echo "this EXP-105  (LES-informed, retrain): KE = ${ke:-?}"
}
