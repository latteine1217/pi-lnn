...
| **CEXP-037** | ✅ `KEY_FINDING` | Re=10031, **hard BC + 固定權重（no GradNorm）+ bc_body=0** | **10.94 %** ✅ | **1.066** ✅ | 8.50 | 8.65 | 10k | ✅✅ **hard BC 路線首次成功 + Finding #6 乾淨證明**：CEXP-016（hard BC+GradNorm）111.6% → CEXP-037（hard BC+固定權重）**10.94%（10× 改善）**，唯一差異是 optimizer → **GradNorm 是 hard BC 失敗的唯一元兇**。L_phys 全程穩定（無爆漲），ke_pred/ref 1.066 不 over-predict。**div 8.65 ≈ DNS reference 8.74** → 散度結構與真實 DNS 一致（CEXP-002 的 div 1.14 << 8.74 是 over-smoothing 假解）|
