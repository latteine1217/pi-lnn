...
EXP-200~268 訓練資料用 **K=100 sparse sensors**。對 2D 域，K 個 sensors 對應的 Nyquist wavenumber 上限：

$$ k_\text{Nyquist}^\text{K} = \sqrt{K/\pi} \approx \sqrt{100/\pi} = 5.64 $$

**Neural network 透過 K=100 sensors 只能觀察到 k ≤ 5.64 的 info**。
...
- **K-Nyquist 推導**: $k_\text{Nyquist}^\text{K} = \sqrt{K/\pi}$ 是 2D 圓盤 area-based estimate。某些文獻用 $\sqrt{K}$ 或更精細的 sample-pattern-dependent 估計；本驗證的 0.05% agreement 對這幾個替代定義都成立（因為實際收斂 envelope 比 K-Nyquist 預測還寬）。
