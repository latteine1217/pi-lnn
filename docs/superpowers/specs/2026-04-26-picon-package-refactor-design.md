# PI-CON Package Refactor — Design (A1 / B2 / G1)
...
`src/picon_kolmogorov.py` is a 1,913-line monolith that mixes runtime utilities, positional encodings, neural-network blocks, model components, the `LiquidOperator` itself, physics residuals, training helpers, config loading, the 683-line `train_picon_kolmogorov` function, and the CLI entry point.
...
- Tests already import individual pieces (`SpatialSetEncoder`, `LearnableFourierEmb`, `make_picon_model_fn`, `observed_channel_prediction`, `TemporalCfCEncoder`, etc.), proving the boundaries exist conceptually but not physically.
- Two import styles already coexist in the wild: `from picon_kolmogorov import …` (most callers) and `from src.picon_kolmogorov import …` (some tests). Any refactor must preserve both.
...
- Decomposing the 683-line `train_picon_kolmogorov` function.
...
|---|---|---|
| **A1** | Pure structural split — keep `train_picon_kolmogorov` as one function | Lowest risk; reproducibility is preserved; train-loop decomposition is a separate, larger effort. |
| **B2** | Rename to `src/pi_con/`, keep `src/picon_kolmogorov.py` as a thin compatibility shim | Aligns the package name with the project identity ("PI-CON"); the shim keeps all 7 existing callers and the `picon-kolmogorov-train` console script working unchanged. |
...
src/
  pi_con/                     # new package
    ...
    config.py                 # ~150 lines — DEFAULT_PICON_ARGS + load_picon_config
    ...
    operator.py               # ~180 lines — LiquidOperator + create_picon_model + make_picon_model_fn
    physics.py                # ~205 lines — NS/Poisson residuals + RAR + scheduling
    training.py               # ~700 lines — train_picon_kolmogorov + main (A1: not decomposed)
  picon_kolmogorov.py           # compat shim: re-exports from pi_con
...
Each row maps a target file to source line ranges in the current `src/picon_kolmogorov.py`. Symbols listed are exhaustive for that file.
...
| `operator.py` | 651–802, 872–899 | `LiquidOperator`, `create_picon_model`, `make_picon_model_fn` |
...
| `config.py` | 1059–1210 | `DEFAULT_PICON_ARGS`, `_find_project_root`, `_resolve_config_path_value`, `load_picon_config` |
| `training.py` | 1211–1913 | `train_picon_kolmogorov`, `main` |

### Rationale for ambiguous placements

- **`make_picon_model_fn` → `operator.py`**: it is a closure factory over `LiquidOperator`; it belongs with the model abstraction it wraps, not with losses or training.
...
- **`DEFAULT_PICON_ARGS` → `config.py`**: it is a defaults table consumed by `load_picon_config`; it is configuration, not training logic, even though `main` and `train_picon_kolmogorov` use it.
...
### `src/pi_con/__init__.py`

Explicit, exhaustive re-export of every symbol that any caller currently imports from `picon_kolmogorov`:
...
from .operator import LiquidOperator, create_picon_model, make_picon_model_fn
from .physics import (
...
from .config import DEFAULT_PICON_ARGS, load_picon_config
from .training import main, train_picon_kolmogorov
...
### `src/picon_kolmogorov.py` (compat shim, replaces the monolith file)
...
- `[project.scripts] picon-kolmogorov-train = "picon_kolmogorov:main"` continues to work because `src/picon_kolmogorov.py` still exists (now as a shim) and still exports `main`.
- `[tool.hatch.build.targets.wheel] packages = ["src"]` continues to package the entire `src/` tree, which now includes `pi_con/`.
...
2. **Dual import smoke:**
   ```bash
   uv run python -c "from picon_kolmogorov import *; from pi_con import *; print('ok')"
   ```
3. **Identity check** (catches the "two copies of the class" bug):
   ```bash
   uv run python -c "
   import picon_kolmogorov, pi_con
   assert picon_kolmogorov.LiquidOperator is pi_con.LiquidOperator
   assert picon_kolmogorov.train_picon_kolmogorov is pi_con.train_picon_kolmogorov
   assert picon_kolmogorov.DEFAULT_PICON_ARGS is pi_con.DEFAULT_PICON_ARGS
   print('identity ok')
   "
   ```
4. **Behavioral equivalence — fixed-seed smoke train:**
   - Before refactor: `uv run python src/picon_kolmogorov.py --config configs/smoke_re1000_uvomega.toml --device cpu` (or mps), capture per-step metrics for the first ~50 steps to a JSON.
...
5. **Console script check:** `uv run picon-kolmogorov-train --help` returns successfully.
...
| Missed symbol in `__init__.py` re-export breaks legacy `from picon_kolmogorov import X` | Low | Explicit listing in shim (no `*`-only); step 1 (full pytest) and step 2 (dual import) catch the obvious cases. |
...
| Console script `picon-kolmogorov-train` breaks | Low | Step 5 verifies; the shim re-exports `main`, so the entry point resolves. |
...
- `train_picon_kolmogorov` function body — A1 boundary.
...
1. New `src/pi_con/` package with 10 files.
2. `src/picon_kolmogorov.py` replaced with the compat shim.
...
Reassess A2 (decomposing `train_picon_kolmogorov`) once this lands. Signals favoring A2:
...
If A2 is undertaken later, it builds on the package boundaries established here: optimizer/scheduler builders go in a new file under `pi_con/training/`, loss orchestration moves to `losses.py`, and the `train_picon_kolmogorov` function shrinks to a coordinator.
